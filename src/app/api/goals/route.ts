...
export async function POST(req: NextRequest) {
  ...
  const { title, description, emoji, category, endDate, totalDays, toolIds = [], color } = body
  ...
  const goalResult = await query(
    `INSERT INTO "Goal" (id, "userId", title, description, emoji, category, "startDate", "endDate", "totalDays", progress, status, color, "lastActivity", "createdAt", "updatedAt")
     VALUES (gen_random_uuid()::text, $1, $2, $3, $4, $5, NOW(), $6, $7, 0, 'active', $8, 'Goal created', NOW(), NOW())
     RETURNING *`,
    [session.user.id, title, description ?? null, emoji ?? "🌱", category ?? "general", endDate, totalDays, color ?? "#3ecfc6"]
  )
  ...
  if (toolIds.length > 0) {
    ...
    await query(`INSERT INTO "GoalTool" ("goalId", "toolId") VALUES ${values} ON CONFLICT DO NOTHING`, params)
  }

  // Spawn dedicated agent
  const agentName = `${title.split(" ").slice(0, 2).join("")}Bot`
  const agentResult = await query(
    `INSERT INTO "Agent" (id, "goalId", "userId", name, avatar, personality, status, "actionsToday", "totalActions", "createdAt", "updatedAt")
     VALUES (gen_random_uuid()::text, $1, $2, $3, $4, $5, 'idle', 0, 0, NOW(), NOW())
     RETURNING *`,
    [goal.id, session.user.id, agentName, emoji ?? "🤖", "Dedicated agent — focused and proactive"]
  )
  ...
  await query(
    `INSERT INTO "ActivityLog" (id, "userId", "agentId", "goalId", "agentName", "goalTitle", action, type, details, "createdAt")
     VALUES (gen_random_uuid()::text, $1, $2, $3, $4, $5, 'Commitment created', 'updated', 'New goal added to EnviroAgent', NOW())`,
