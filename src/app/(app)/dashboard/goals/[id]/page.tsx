...
import { GOALS, AGENTS, TOOLS, ACTIVITY, TYPE_COLORS, TYPE_BG } from "@/components/app/mock-data"
...
export default function GoalDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const activity = ACTIVITY.filter(a => a.goalId === goal.id)
            ...
            <Section title="Commitment horizon">
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px", marginTop: "16px" }}>
                {[
                  ...
                  { label: "Progress", value: `${goal.progress}%`, color: goal.color },
                ].map(m => (
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                {tools.map(tool => (
                  <div key={tool.id} style={{
                    ...
                    <div>
                      <div style={{ fontSize: "0.8rem", fontWeight: 500, color: "var(--app-text-1)" }}>{tool.name}</div>
                      <div style={{ fontSize: "0.68rem", color: "var(--app-text-3)" }}>{tool.description}</div>
                    </div>
                  </div>
                ))}
              </div>
