...
import { TOOLS, GOALS, AGENTS } from "@/components/app/mock-data"
    ...
    <div style={{ minHeight: "100dvh", padding: "24px", maxWidth: "900px" }}>
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: "12px" }}>
          {filtered.map((tool, i) => {
            const usedByAgents = AGENTS.filter(a => tool.usedBy.includes(a.id))
            const usedByGoals = GOALS.filter(g => usedByAgents.some(a => a.goalId === g.id))
