...
import { type ApiGoal, type ApiActivityItem } from "@/lib/goal-types"
...
export function DashboardClient({ goals }: Props) {
  ...
  const totalActionsToday = goals.reduce((s, g) => s + (g.agent?.actionsToday ?? 0), 0)

  // Collect the most recent activity across all goals (goals list includes agents, not activity)
  const recentActivity: ApiActivityItem[] = goals
    .flatMap(g => g.activity ?? [])
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            <div style={{
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginTop: "12px" }}>
                {goals.filter(g => g.agent).map(goal => {
                  const agent = goal.agent!
                  return (
                ...
                {goals.filter(g => g.agent).length === 0 && (
                  <p style={{ fontSize: "0.78rem", color: "var(--app-text-3)", margin: 0 }}>No agents yet.</p>
                )}
              </div>
            </div>
            ...
            <div style={{
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginTop: "12px" }}>
                {recentActivity.length === 0 && (
                  <p style={{ fontSize: "0.78rem", color: "var(--app-text-3)", margin: 0 }}>No recent activity.</p>
                )}
                {recentActivity.map(item => (
                  <div key={item.id} style={{ display: "flex", gap: "10px", alignItems: "flex-start" }}>
                    ...
                    <div>
                      ...
                      <p style={{ fontSize: "0.7rem", color: "var(--app-text-3)", margin: "2px 0 0" }}>{item.agentName} · {formatTime(item.createdAt)}</p>
                    </div>
                  </div>
                ))}
              </div>
