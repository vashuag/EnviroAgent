...
import type { ApiActivityWithGoal } from "@/lib/goal-types"
import Link from "next/link"

const TYPES = ["all", "scheduled", "drafted", "blocked", "nudged", "logged", "updated", "system", "user"] as const
type ActivityType = typeof TYPES[number]

interface GoalStub { id: string; title: string; emoji: string | null }

function groupByDate(items: ApiActivityWithGoal[]) {
  const groups: Record<string, ApiActivityWithGoal[]> = {}
  for (const item of items) {
    const date = new Date(item.createdAt).toLocaleDateString("en-IN", { weekday: "long", day: "numeric", month: "long" })
...
export default function ActivityClient({ activity, goals }: { activity: ApiActivityWithGoal[]; goals: GoalStub[] }) {
          ...
          <div style={{ display: "flex", gap: "4px", flexWrap: "wrap" }}>
            {TYPES.map(t => (
              <button key={t} onClick={() => setFilter(t)} style={{
                ...
                background: filter === t ? (t === "all" ? "var(--app-teal)" : (TYPE_BG[t] ?? "var(--app-card)")) : "var(--app-card)",
                border: filter === t ? (t === "all" ? "1px solid var(--app-teal)" : `1px solid ${(TYPE_COLORS[t] ?? "#3ecfc6")}40`) : "1px solid var(--app-border)",
                color: filter === t ? (t === "all" ? "#070f1c" : (TYPE_COLORS[t] ?? "#3ecfc6")) : "var(--app-text-3)",
                ...
                {items.map((item, i) => (
                  ...
                  >
                    ...
                    <span style={{
                      ...
                      background: TYPE_BG[item.type] ?? "rgba(62,207,198,0.12)",
                      color: TYPE_COLORS[item.type] ?? "#3ecfc6",
